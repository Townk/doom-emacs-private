...
#+SUBTITLE: A detailed setup for Emacs using Doom Emacs configuration framework
#+AUTHOR: Thiago Alves
#+EMAIL: thiago@rapinialves.com

#+keywords: emacs config literate doom
#+language: en
#+conseption_date: Fri, Jan 1 2021
...
#+setupfile: .github/support/org/html-export.setup
...
This configuration uses [[https://github.com/hlissner/doom-emacs][Doom Emacs]] configuration framework (from now on, referred as Doom) as its base, so, my suggestion to you is to get familiarized with it *before* diving deep into this document (a good starting point is the [[https://github.com/hlissner/doom-emacs/blob/develop/docs/getting_started.org][Getting Started]] page on the official repository), there will be options, functions, and variables I will use without much explanation because they're some sort of /common knowledge/ in the Doom's community.

Furthermore, when you install Doom for the first time, it creates the =config.el= and =packages.el= files for you, containing some useful doc-strings there. In this version of my configuration, I will try to include as much of those comments as possible as /free text/. I want you to remember that I didn't come up with those words, some folks much smarter than me did, so feel free to [[https://liberapay.com/hlissner][thank]] the Doom's [[https://paypal.me/henriklissner][creator]].

For instance, a Doom installation without a previous private configuration creates the file =$DOOMDIR/config.el= with the following comments:
...
I intend to start with features I consider common to any text editor, then, I'll go into some personal preferences still related to text editor in general. Once I'm done with the common features, I'll configure Emacs for writing prose. I decided to do all my prose writing using [[https://orgmode.org/][Org Mode]] and will focus the configuration, at that point, to adjust Emacs and Org to fit my personal workflow.

After configuring Emacs for writing prose, my next focus is programming (which is what I do for work). I will set up [[https://www.gnu.org/software/emacs/manual/html_node/elisp/][Emacs Lisp]], [[https://www.javascript.com/][Javascript]]/[[https://www.typescriptlang.org/][Typescript]], [[https://kotlinlang.org/][Kotlin]], [[https://www.python.org/][Python]], [[https://www.shellscript.sh/][Shell Script]], [[https://developer.apple.com/swift/][Swift]], [[https://golang.org/][Go]], [[https://www.rust-lang.org/][Rust]], and [[http://www.open-std.org/JTC1/SC22/WG14/][C]]/[[http://www.open-std.org/JTC1/SC22/WG21/][C++]]. Most likely not in this order and some of them in more details than others, but at a minimum, I want Emacs to open files of those languages without errors or wrong syntax highlight.
...
A long time ago I discovered [[http://literateprogramming.com][Literate Programming]] when I first read an Emacs configuration using Org Mode. It was the [[https://pages.sachachua.com/.emacs.d/Sacha.html][Emacs configuration]] of [[https://sachachua.com/blog/about/][Sacha Chua]], and it was a fantastic reading. I tried a couple of times to do the same with past configurations without much success.

I believe the mentality shift happened when I finally decided to read the original paper about [[http://literateprogramming.com/knuthweb.pdf][Literate Programming]] by [[https://www-cs-faculty.stanford.edu/~knuth/][Donald E. Knuth]], and just like that, I wanted to practice Literate Programming on about everything I do.
...
** About Doom configuration style
...
*** Doom has its package manager

Doom does not use =package.el=, it has its package manager system and even though, it uses ~use-package~ and ~straight.el~ packages, it uses them as support tools for the package manager to work. This means that if you copy and paste a configuration showing how to use them into your =config.el=, most likely you will get some sort of error. So, keep this in mind when you blindly copy a snippet to your =config.el=.

*** Lazy load everything

You should get used to use the options =:defer=, =:hook=, =:after=, =:mode=, and =:commands= from ~use-package~ (check the [[https://github.com/jwiegley/use-package][package documentation]] for more details) when configuring any package with it. Not doing so, may cause the package to get loaded during the Emacs initialization, potentially slowing the start of Emacs.

*** Configuration evaluation order

There are four potential loading /spots/ for each configuration you want to tweak, and you must always decide which one is the best option to apply in your particular case:

- Directly on =config.el= :: This is the most direct point of configuration. As Emacs evaluates the line of your configuration, it applies such configuration immediately. This is a good spot to configure variables the documentation states that /“it needs to get configured before the package gets loaded”/. On Doom's world, any directory or file variable should go to directly into your =config.el=;
- Inside the =:init= section of ~use-package~ :: Depending on how a package gets configured, this is equivalent to configuring something directly on the =config.el=, but not always. If a package configuration uses the =:after= section from ~use-package~, Emacs evaluates the code inside =:init= only *after* the resource defined on the option, loads;
- Inside the =:config= section of ~use-package~ :: This always executes after the given package gets loaded. You need to consider other instances of the call to ~use-package~ on the same package. The order in which they evaluate dictates who will overwrite a variable last;
- Inside an ~after!~ or ~with-eval-after-load~ macro :: Code here executes after the defined resource gets loaded, but also after all instances of ~use-package~ finish to run code from their =:config= section;

If you have any question about where you should add your configuration, I suggest following this rule of thumb:

- If a package requires you to configure a variable before Emacs loads such package, set the variable directly on the =config.el=;
- If you're configuring a package that Doom installs as part of a module, and it does not require you to define the configuration before Emacs loads the package, use the ~after!~ macro;
- If you're adding a new package to your configuration, configure it using the ~use-package~ macro. To know if you should use ~use-package~ just check if you had to add the package to your =packages.el= file. If you did, then use ~use-package~.

For instance:

#+caption: Standard configuration of a package on Doom
...
*** Use the ~map!~ macro to bind keys
Eventually, you will realize that ~use-package~ has a nice =:bind= option to it, and you may think that your configuration would be so much cleaner if you could have everything under a ~use-package~ call (been there, done that). Unfortunately, this is not a good idea, unless you're aware of all possible gotchas related to =leader= and =localleader=.

My recommendation to you is to always configure your keybindings using the ~map!~ macro. Check its documentation for more details (on Emacs press =SPC h f map! RET=).
...
I would also like to thank [[https://github.com/gongzhitaao][Zhitao Gong]] for the remarkable theme I'm using in this publication, it's called [[https://github.com/gongzhitaao/orgcss][OrgCSS]] and you should definitely check it out. If you try [[https://github.com/gongzhitaao][Zhitao Gong]]'s theme, and it does not look like this, you can use my extra modification by adding the following on your Org preamble:

#+begin_src text :tangle no
,#+setupfile: https://raw.githubusercontent.com/Townk/doom-emacs-private/master/.github/support/org/html-export.setup
#+end_src
...
By the way, if you are reading this document online, there is a chance you might feel a familiarity with the typography used here. This is due to the fact I'm using the same font used by [[https://github.com/mickeynp][Mickey Petersen]] on his amazing [[https://www.masteringemacs.org/][Mastering Emacs]] site. So if you think this is a nice typography for reading online, again, I can't take credit for that either, check Mickey's site and maybe purchase a copy of his [[https://www.masteringemacs.org/order][book]].
...
** Requirements
:PROPERTIES:
:CUSTOM_ID: standard-requirements
:END:

This part of the document will deal with configurations and functionalities that I consider as essential on any text editor, from IDEs to editors like [[https://en.wikipedia.org/wiki/Microsoft_Notepad][Notepad]], even though they're not present in all of them.

*** Modal editing
I need to come clean to the reader about my experiences with text editors. I'm a /die-hard/ Vim user, even though I'm not using Vim for some time now. Nevertheless, I, personally, believe that modal editing is a far superior way to use a text editor than any other alternative, and that is why I consider /modal editing/, a feature that any text editor should have.
...
For instance, if you configure your editor to search for files when you press '=SPC f f=', when you press '=SPC f=' and wait. I want the editor to display all the possible options for me to complete your key sequence.
...
I also want my text editor to allow me to start this remote editing by picking a server and a remote path, so it can create a remote /project/ for me to explore.
...
For some Emacs functions, such source code is a /C/ function from its core code, and by default, Emacs does not know where to look for its own source code.
...
When Emacs is showing line numbers, it does so on the window's left /gutter/. To save some screen space, Emacs only reserves the necessary space on its gutter, to display the biggest visible line number. I believe this behavior disrupts the user experience when I scroll from line 99 to 100 and back. The visual effect is the editor screen moving right and left as I scroll.
...
#+begin_src plantuml :file assets/org-dir-struct.svg :tangle no
@startsalt
...
#+name: fig:org-dirs
#+RESULTS:
[[file:assets/org-dir-struct.svg]]
...
- =agenda= :: The root folder for task management:
  ...
  + =tasks= :: Directory to store Org files with tasks. The main file, which I will track on Org Agenda is the =todo.org=, but I have this extra file called =someday.org= as a dump for tasks I wish to accomplish but are not essentials;
...
** Requirements
Different from the editor requirements on section [[#standard-requirements]], my writing requirements are more loosely defined, and one requirement might need multiple configurations while others might get configured on a single section. Nevertheless, I split my requirements into four distinct stories.

*** Fonts
Fonts for writing must use variable pitch and follow the typography rules defined on section [[#prefs-typography]]. Any reference to code or code elements must get the normal /coding/ style with fixed pitch. Also, other than headlines, element should have variable size depending on their importance.

*** Writing quality
Throughout the /writing sections/ you'll see a common theme: I'm not a good writer. I sure do try my best, and use whatever tools are available to me, but I'm far from the person who sits down and put on paper what is in her head.

My writing quality requirement tries to specify the features that help me be /less bad/ at writing.

- Spell checking :: I need a good spell checking dictionary that offers English and Portuguese (my native language) support;
- Grammar checking :: The title says it all. I need something on the molds of [[https://www.grammarly.com/][Grammarly]] or [[https://languagetool.org/][LanguageTool]]. I would be fantastic to have such tool supporting Portuguese, but as far as I know, these tools only support English, so I would be OK if I get grammar check for English only;
- Dictionary :: I need a good words dictionary that support thesaurus, etymology, and translation between English and Portuguese;
- Text linting :: This is a tool closer to programming languages, but there are certain text lints that can help you write a better text. At a minimal, I want my text lint to support:
  - Insensible words;
  - Weasel words;
- Revision :: I want to have some sort of tool that allows me to add comments on text files. I'll use this feature when I'm reviewing the text with a group of people and I want to add people's comments as annotations to the text;
...
- Posting on my blog (using [[https://gohugo.io/][Hugo]] export);
- Publishing the text into a wiki (which I have a requirement to support [[https://www.xwiki.org/xwiki/bin/view/Main/WebHome][XWiki]] and [[http://www.confluense.com/][Confluense]]);
- Exporting the text to a book format (PDF and [[https://www.w3.org/AudioVideo/ebook/][ePub]]);
- Exporting the text to an article format (using [[https://www.latex-project.org/][LaTeX]]);
- Exporting the text as an HTML presentation (using reveal.js);
- Sending an email with the text attached as a document (using any of the previous supported formats);
...
** Writing typography
Fortunately for me, ~org-mode~ and other writing major modes, derive from the built-in ~outline-mode~. This allows me to configure the headline typography for ~outline-mode~ and have ~org-mode~ inheriting it.

To start, I will make sure the headlines of any mode derived from ~outline-mode~ gets the correct face configuration:
...
I like to have secondary information (anything that is *not* the main text) using a smaller face:
...
The last configuration related to fonts, is how I like to render certain document elements. Instead of showing the standard Org markup, I like to use Unicode characters instead. The table [[tbl:org-font-lock]] shows the mapping between Org elements and the respective Unicode character I want to use:

#+caption: Org elements to unicode characters
#+name: tbl:org-font-lock
| Org element          | Org symbol | Unicode |
|----------------------+------------+---------|
|                      |    <c>     |   <c>   |
| Unmarked checkbox    |   =- [ ]=    |    ☐    |
...
Unfortunately, I can only define new ~font-lock-keywords~ for a major mode, after loading it for the first time, so I will effectively add them when I'm configuring Org on the next section. But here, I want to define a constant holding the list of new ~font-lock-keywords~ that I can use later during the Org configuration. Another advantage to have such constant, is to allow me to reuse the same configuration on other derived modes (e.g. ~org-journal-mode~):
...
Even though our configuration can render math formulas nicely, it's still awkward to edit them once you write them. To help us with this task, I'll install a plugin called [[https://github.com/io12/org-fragtog][~org-fragtog~]]. This plugin will show the math formula with the LaTeX syntax if the cursor is inside the formula area, and will render the formula when the cursor leaves it.
...
To fix this behavior, we just need to re-bind some movement keys to the /visual/ version of their original command, for instance, while we have =j= bound to ~evil-next-line~, in ~org-moode~, we want it bound to ~evil-next-visual-line~:
...
(map! :after org :map org-mode-map
      :localleader
      :desc "org-reveal" :n "R" #'org-reveal
      :desc "Copy visible" :v "y" #'org-copy-visible)
#+end_src

#+caption: $DOOMDIR/config.el
#+begin_src emacs-lisp
(map! :after org :map org-mode-map
      :localleader :prefix ("s" . "tree/subtree")
      :desc "Clone subtree" :n "c" #'org-clone-subtree-with-time-shift
      :desc "Copy special" :n "y" #'org-copy-special
      :desc "Cut special" :n "d" #'org-cut-special
      :desc "Cut special" :n "x" #'org-cut-special)
#+end_src
...
(map! :after org :map org-mode-map
      :prefix ("g" . "action")
      :desc "org-ctrl-c-ctrl-c" :n [return] #'org-ctrl-c-ctrl-c
      :desc "org-ctrl-c-ctrl-c" :n "RET" #'org-ctrl-c-ctrl-c)
#+end_src
...
(map! :after org :map org-mode-map
      ...
      :n "s-C-h" #'org-up-element
      :n "s-C-j" #'org-forward-heading-same-level
      :n "s-C-k" #'org-backward-heading-same-level
...
#+caption: $DOOMDIR/config.el
#+begin_src emacs-lisp
(map! :after org :map org-src-mode-map
...
#+begin_src emacs-lisp
(custom-set-faces!
  `(langtool-errline :background "#372C34"
...
#+caption: $DOOMDIR/config.el
#+begin_src emacs-lisp
(defconst my-roam-dailies-file-name-base "daily/daily-notes-%<%Y-%m-%d>"
  "Filename format for the Daily Notes file.")

(defconst my-roam-dailies-file-name (format "notes/%s.org" my-roam-dailies-file-name-base)
  "Filename format for the Daily Notes file.")
...
#+caption: autoload/writing.el
#+begin_src emacs-lisp :tangle autoload/writing.el
;;;###autoload
(defun my-dailies-find (tag)
  "Open the right daily note based on the given TAG.

Accepted values for TAG are:

- \"yesterday\"
- \"today\"
- \"tomorrow\"

If TAG is not one of these values, this function will raise an error."
  (pcase tag
    ("yesterday"
     (format "file:%s"
             (string-trim
              (expand-file-name
               (let ((org-capture-plist
                      `(:default-time ,(org-time-string-to-time (org-read-date nil nil "-1")))))
...
#+begin_src emacs-lisp
(after! org
  (pushnew! org-link-abbrev-alist '("dailies" . "%(my-dailies-find)"))
  (org-add-link-type "info" 'org-info-command))
#+end_src
...
(after! org-roam
  (setq org-roam-dailies-capture-templates
        `(("d" "default" entry #'org-roam-capture--get-point "* %?"
           :file-name ,my-roam-dailies-file-name-base
           :head ,my-roam-dailies-header))))
#+end_src
...
;; (use-package! nroam
;;   :hook '(org-roam-mode . nroam-setup-maybe))
...
- [[fig:org-dirs][Figure 2: My org directory structure]]
