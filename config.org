...
* TODO Editor standard features [5/12]
...
*** Rich keybindings
Any half-decent text editor these days supports an infinite pletora of shortcuts. What I believe an editor should support to be ahead of the game, is the ability to bind *any* editor action to a key, and to offer to the user some sort of guidance while one uses the editor.
...
On Emacs, the package responsible to provide Vim inside Emacs is [[https://github.com/emacs-evil/evil][Evil]], and from all Vim plugins for other editors that I use (and they were not few), Evil is by far the best. So much that compare it with other Vim plugins is do injustice to Evil developers. More often than not, we call Evil the Vim inside Emacs.
...
Where /operation/ is the command you're executing, operator can be /i/ (inside) or /a/ (also), and the /target object/ can be any of the targets defined Evil (e.g. /s/ for /sentense/, /p/ for /paragraph/, /[/ for /square brackets content/, etc. ).
...
IDE [[https://www.jetbrains.com/idea/][IntelliJ IDEA]]. I bring this up not to chastise who uses or to complain about it. In face, InjelliJ the best IDE for Java and Kotlin out there. It has some features that are so good, that I want to implement on any text editor I use. One of these features is what I call /Smart Backspace/.
...
And then, after loading Evil, we can hook thes functions to a mode switch:
...
The aggressive auto-save will work as expected, but it will crate a side effect with my literate config. Since Doom tangles the Org file every time we save it, the aggressive auto-save will make this tangle happen more often than we want.
...
That works, but now we have to do a =doom sync= for every change on our config. Even if the change didn't modified anything on =packages.el=, =init.el=, or any of the /autoload/ files.
...
** TODO Remote editing
* TODO Editor personal preferences [0/9]
** TODO Requirements
...
;;;###autoload
(defadvice! +doom-close-window-when-no-buffer (&rest _)
...
#+caption: config.el
#+begin_src emacs-lisp :padline no
  (custom-set-faces! '(selectrum-current-candidate :background "#3c3c3c"))
#+end_src
...
This last function needs another helper one, to make sure it returns the correct name for a session on a given indes:
...
(defun +doom-adjust-org-fonts-h ()
  (after! org
    (set-face-attribute 'org-document-title nil :height 2.0 :weight 'light)
    ...
    (set-face-attribute 'org-block-begin-line nil :inherit 'fixed-pitch :height 0.8)
    (set-face-attribute 'org-block-end-line nil :inherit 'org-block-begin-line)
    (set-face-attribute 'org-tag nil :height 150)
    (set-face-attribute 'org-level-1 nil :height 1.4 :weight 'normal :slant 'normal)
    (set-face-attribute 'org-level-2 nil :height 1.3 :weight 'normal :slant 'italic)
    (set-face-attribute 'org-level-3 nil :height 1.2 :weight 'normal :slant 'italic)
    (set-face-attribute 'org-level-4 nil :height 1.1 :weight 'bold   :slant 'normal)
    (set-face-attribute 'org-level-5 nil :height 1.1 :weight 'bold   :slant 'normal)
    (set-face-attribute 'org-level-6 nil :height 1.1 :weight 'bold   :slant 'normal)
    (set-face-attribute 'org-level-7 nil :height 1.1 :weight 'bold   :slant 'normal)
    (set-face-attribute 'org-level-8 nil :height 1.1 :weight 'bold   :slant 'normal))
...
- Focus writting :: When I need to dive deep into creating a document, it's useful to remove distractions from my writing environment. Besides getting rid of UI elements, I would like to have the same /Focus Mode/ behavior from [[https://ia.net/writer][iA Writer]];
...
**** TODO Move macro snippet to the xwiki section on  [[Publishing]]
...
** TODO Presenting
- Use =reveal.js= inside a WebKit XWidget in fullscreen
...
- Triage :: This process is to mark done tasks as done, and adjust any deadlines or other information on my tasks:
  + Daily :: Should be a quick 5 min check on the tasks done during the day. I will use this time to also add unexpected tasks on my list so I can have amore accurate record of my work day;
...
- Check if the project has one of the recognized files and open it, in this order (we should look for four extensions on each one of the files, also in this order: =.org=, no extension, =.md=, and =.rst=):
  ...
  + =NEWS=
  + =README=
- If none of the recognizeable files is present, run ~project-find-file~;
