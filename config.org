...
For instance, when one installs Doom without a previous private configuration, the installer creates the file =$DOOMDIR/config.el= with the following comments:
...
After configuring Emacs for writing prose, I will focus this document on programming (which is what I do for work). I will cover [[https://www.gnu.org/software/emacs/manual/html_node/elisp/][Emacs Lisp]], [[https://www.javascript.com/][Javascript]]/[[https://www.typescriptlang.org/][Typescript]], [[https://kotlinlang.org/][Kotlin]], [[https://www.python.org/][Python]], [[https://www.shellscript.sh/][Shell Script]], [[https://developer.apple.com/swift/][Swift]], [[https://golang.org/][Go]], [[https://www.rust-lang.org/][Rust]], and [[http://www.open-std.org/JTC1/SC22/WG14/][C]]/[[http://www.open-std.org/JTC1/SC22/WG21/][C++]]. Most likely not in this order and some of them in more details then others, but at a minimum, I want Emacs to open files of those languages without errors or wrong syntax highlight.
...
Even though Doom uses =use-package= and =streight.el=, it uses them as support tools for the package manager to work. This means that if you try to copy a configuration using these packages into your private configuration, most likely you will get some sort of error. So, keep this in mind when you blindly paste some configuration in your =config.el=.

If you want to configure a package using =use-package=, use the ~use-package!~ (with the exclamation point at the end) macro to do so. This macro make sure nothing gets loaded if you disable a package on =packages.el=, and it allows you to use multiple calls of it (it also has some special options you can use, so you should definitely check its documentation).

The second and the most important point is the order a configuration loads. There are four loading spots for each package, and you must always decide which one is the best option to apply your particular configuration:
...
- Inside the ~:init~ section of ~use-package!~ :: tbd;
- Inside the ~:config~ section of ~use-package!~ :: tbd;
- Inside an ~after!~ macro :: tbd;
