...
If you, like me, tinker with your config until it breaks, is important that we have some tools to help us figure out what is going on. Regardless if you'll use this configuration /ipsis litteris/, or start yours from scratch, and copy the tidbits that may interest you. Before you start know these things:
...
#+caption: config.el
#+begin_src emacs-lisp
(setq +doom-dashboard-banner-dir (expand-file-name "banner" doom-private-dir)
...
(defun my-company-indent-or-complete-common (arg)
  ...
  (if (or (not yas-minor-mode)
      ...
      (cond ((use-region-p) (indent-region (region-beginning) (region-end)))
            ;; ((memq indent-line-function '(indent-relative indent-relative-maybe)) (company-complete-common))
            (t (let ((old-point (point))
                 ...
                 (when (and (eq old-point (point))
                   ...
                   (unless (pcase major-mode
                             ('org-mode (or (org-indent-line) (org-cycle-level) (org-cycle-item-indentation)))
...
Last, I ned to setup the proper keymaps for the package:
...
** Evil
This section will show the reader, some of the small changes I decided to implement on Doom related to Evil.

For most of it, Doom integration of Evil is pretty amazing, I found just one package I don't use (or don't want to use at all), one configuration that I want to do some adjustments, and three packages missing.
...
The Evil configuration I want to adjust to my liking is on =evil-goggles=. I understand Henrik (Doom's creator) has a good rational on why he setup =evil-goggles= the way it is, but I like the original design where *all* changes get the highlight treatment.
...
And for the packages I'm missing, they are:
...
I found the =evil-string-inflection= to have one of the best integrations with Evil, so I will stick with it for a while to evaluate things:
...
#+caption: config.el
#+begin_src emacs-lisp
(map! :gi [s-return]   #'+default/newline-below
...
One binding I feel is missing on Doom, is the hability to evaluate any sexp on *any* mode, like a /global/ ~eval-last-sexp~. Gladly, it's easy enough to create my own:
...
I don't remember when this shortcut enter my life, but some time ago I got used to use =s-RET= to /confirm operations/ like sending emails and messages. In order to try to replicate this behavior I will start setting it to the ~with-editor-mode-map~ together with the analog cancel operation with =s-ESC= shortcut:
...
(after! info
  (custom-theme-set-faces! 'doom-one
    ...
    `(info-glossary-word :foreground ,(doom-color 'yellow 256)
                         ...
                         :box (:line-width (4 . 1) :color ,(doom-color 'yellow-bg 256) :style nil)
                         :inherit fixed-pitch)
...
- Ivy :: Created as a lightweight version of Helm, with the goal to achieve parity with Help at a minium;
...
- Configure result item annotations (to display exta information for each completion candidate);
...
The idea is to install =prescient= but not call ~(selectrum-prescient-mode 1)~. Instead, setup the two required hooks for remembering, and the one variable, later on =selectum=, to use the /pre-processing/ from =prescient=. So let's start with =packages.el=:
...
(use-package! flyspell
  :hook (spell-fu-mode . flyspell-mode))
...
#+caption: autoload/development.el
