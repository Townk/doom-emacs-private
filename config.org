...
Doom does not use =package.el=, it has its own package manager system and even though, it uses =use-package= and =streight.el= packages, it uses them as support tools for the package manager to work. This means that if you copy and paste a configuration showing how to use them into your =config.el=, most likely you will get some sort of error. So, keep this in mind when you blindly copy a snippet to your =config.el=.
