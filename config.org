...
This section describes some aspects of Doom that I felt important to understand before I was able to properly configure it.

The first thing to keep in mind is:

/"Doom has its own package manager"/.

Even though Doom uses =use-package= and =streight.el=, it uses them as support tools for the package manager to work. This means that if you copy and paste a configuration that uses one of these packages into your =config.el=, most likely you will get some sort of error. So, keep this in mind when you blindly copy a snippet to your =config.el=.

If you want to configure a package using =use-package=, use the ~use-package!~ (with the exclamation point at the end) macro to do so. This macro make sure nothing gets loaded if you disable a package on =packages.el=.

The second thing is:

/"Lazy load *everything*!"/

You should get used to use the options =:defer=, =:hook=, and =:commands= from ~use-package!~ when configuring any package this way. Not doing so, may cause the package to get loaded during the Emacs initialization, potentially slowing the start of Emacs.

The third and the most important point is the order a configuration loads. There are four loading spots for each package, and you must always decide which one is the best option to apply your particular configuration:

- Directly on =config.el= :: This is the most direct point of configuration. As Emacs read the line of your configuration, it applies such configuration immediately. This is a good spot to configure variables the documentation states that /"it needs to get configured before the package gets loaded"/;
- Inside the ~:init~ section of ~use-package!~ :: Depending on how a package gets configured, this is equivalent to configuring something directly on the =config.el=, but not always. If a package configuration uses the =:after= section from ~use-package!~, the code inside =:init= gets evaluated only *after* the resource loads;
- Inside the ~:config~ section of ~use-package!~ :: This always execute after the given package gets loaded. You need to consider other instances of the call to ~use-package!~ on the same package. For instance, if you're configuring a package Doom also configure, the you add to the =:config= section may execute before Doom's own configuration, which can potentially erase your configuration with Doom's defaults;
- Inside an ~after!~ macro :: Code here executes after the resource gets loaded, but also after all instances of ~use-package!~ finish to run code from their =:config= section;

If you have any question about where you should add your configuration, I suggest to follow the following rule of thumb:

- If a package requires you to configure a variable before Emacs loads it, set the variable directly on the =config.el=;
- If you're configuring a package that Doom installs as part of a module, and it does not required to define the configuration before Emacs loads the package, use the ~after!~ macro;
