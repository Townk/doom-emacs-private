...
Most snippet frameworks available accross editors (including /Yasnippet/) offer the ability to edit some placeholders on the snippet so one can have a single snippet and use it on different scenarios. For instance:
...
What we want to do here, is to delete the placeholder and move to the next posittion, and end up with the same result as before:
...
Throughout this document, you will find helpful snippets I like to define for certain features. I decided to spread the snippets accross different features because I want to indicate to the reader to consider the snippet in question as an integral part of the configuration at hand.

** TODO Completion framework
Some times, I come across a new package or a set of them that I keep my eyes on it. After a while, such packages start showing their value and start getting adopted by the community. When that time comes, in some cases, I want to adopt them as well.

This happen with a set of packages I kindly named /Selectrum and Friends/. The list of packages that compose this set are:
...
I can describe the process to configure these packages as follows:

- Configure Emacs completion system filtering and mach highlights;
- Configure Emacs completion system candidates sorting;
- Configure completioin candidates annotations (to display exta information for each completion candidate);
- Configure the completion UI;
- Configure contextual menu;
- Configure extra commands;
- Alter the standard behavior Doom Emacs defines, to fit my preferences;
...
(use-package! orderless
  ...
  :config
  (custom-theme-set-faces! 'doom-tomorrow-nitght
...
#+begin_src emacs-lisp :tangle autoload/selectrum.el
;; ;;;###autoload
...
;;   (let ((orderless-match-faces-original orderless-match-faces))
;;     (setq orderless-match-faces [completions-common-part])
;;     (apply fn args)
;;     (setq orderless-match-faces orderless-match-faces-original)))
#+end_src
...
(use-package! aggressive-indent
 ...
 (add-hook! '(emacs-lisp
              ...
              shell
              javascript
              typescript) 'aggressive-indent-mode))
#+end_src
...
#+caption: autoload/evil.el
#+begin_src emacs-lisp :tangle autoload/evil.el
;;;###autoload
...
(add-hook! 'doom-load-theme-hook
  (defun my-themes-add-extra-colors-h ()
...
(setq doom-font (font-spec :family "iMWritingMonoS Nerd Font" :size 14 :weight 'semi-light)
      doom-big-font (font-spec :family "iMWritingMonoS Nerd Font" :size 32 :weight 'semi-light)
      doom-variable-pitch-font (font-spec :family "iA Writer Quattro V" :size 18)
      )
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/appearances.el
;;;###autoload
...
#+begin_src emacs-lisp :tangle autoload/appearances.el
;;;###autoload
(defun my-get-icon (icon &optional color)
...
Check documentation of the function `my-get-raw-icon' for a
list of supported icons."
  (cond ((eq color 'raw) (my-get-raw-icon icon))
        (color (propertize "--"
                   'face `(:foreground ,color)
                   'display (my-get-raw-icon icon)))
        (t (propertize "--"
...
#+caption: autoload/tabs.el
#+begin_src emacs-lisp :tangle autoload/tabs.el
;;;###autoload
...
#+begin_src emacs-lisp
  ...
  (setq tab-line-tab-name-function #'my-tabs-name-buffer)
...
#+begin_src emacs-lisp :tangle autoload/editor.el
;;;###autoload
(defun my-window-single-buffer-p (&optional win)
...
;;;###autoload
(defun my-close-window-when-kill-buffer (&optional buffer-or-name)
  (let* ((buf (get-buffer (or buffer-or-name (current-buffer))))
         (win (get-buffer-window buf)))
    (when (and win
               (my-window-single-buffer-p win))
      (delete-window win))))
...
(add-hook 'kill-buffer-hook 'my-close-window-when-kill-buffer)
...
(add-hook! (help-mode
            ...
            Info-mode
            lexic-mode)
  (defun my-popup-format-info-side-pannel-h ()
...
#+begin_src emacs-lisp
      (:when IS-MAC
       :ne "s-1"   #'+workspace/switch-to-0
       :ne "s-2"   #'+workspace/switch-to-1
       :ne "s-3"   #'+workspace/switch-to-2
       :ne "s-4"   #'+workspace/switch-to-3
       :ne "s-5"   #'+workspace/switch-to-4
       :ne "s-6"   #'+workspace/switch-to-5
       :ne "s-7"   #'+workspace/switch-to-6
       :ne "s-8"   #'+workspace/switch-to-7
       :ne "s-9"   #'+workspace/switch-to-8
       :ne "s-0"   #'+workspace/switch-to-final)
#+end_src
...
;;;###autoload
(defun my-adjust-outline-fonts-h ()
  (after! outline
    (custom-theme-set-faces! 'doom-one
...
(after! outline
  (if (daemonp)
      (add-hook! 'server-after-make-frame-hook #'my-adjust-outline-fonts-h)
    (my-adjust-outline-fonts-h)))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/org.el
;;;###autoload
(defun my-adjust-org-fonts-h ()
  (after! org
    (custom-set-faces!
      '(org-document-title :height 2.0 :weight light :inherit variable-pitch)
...
#+begin_src emacs-lisp
  (if (daemonp)
      (add-hook 'server-after-make-frame-hook #'my-adjust-org-fonts-h)
    (my-adjust-org-fonts-h))
#+end_src
...
#+caption: config.el
#+begin_src emacs-lisp
(add-hook 'org-mode-hook #'my-org-config-h)
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/editor.el
;;;###autoload
(defun my-get-aspell-cmd-flags (&optional camel-case &rest extra-args)
...
(after! ispell
  ...
  (setq ispell-extra-args (my-get-aspell-cmd-flags t))
  ...
  (add-hook! org-mode
    (defun my-spell-add-extra-ignores-to-org-h ()
      ...
      (setq-local ispell-extra-args (my-get-aspell-cmd-flags nil
...
#+begin_src emacs-lisp :tanglexupeta.el :mkdirp yes
...
#+begin_src emacs-lisp :tangle autoload/capture.el
...
(defun my-capture-projects-p ()
  "Retuns true when current buffer belongs to current open
project."
    ...
    (if icon
        (progn
          (unless (and (consp icon)
                       (memq (car-safe icon) my-supported-icons)
          ...
          (format "%s %s" (my-get-icon (car icon) (cdr icon)) text))
      text)))
#+end_src
...
ARGS should all be string that will get concatenated with no
spaces between them. Default to \"%?\"."
...
(defun my-capture-heading-prefix ()
  "Returns a string of `*` characters representing an org header
level. The number of characters is the one on the org-capture
`:heading-level' property or calculated by the org-capture `:olp'
property size."
...
#+caption: autoload/roam.el
#+begin_src emacs-lisp :tangle autoload/roam.el
;;;###autoload
...
(defconst my-roam-dailies-file-name "notes/daily/daily-notes-%<%Y-%m-%d>.org"
  "File name format for the Daily Notes file.")
...
#+caption: share/capture/org-mode/capture-support
...
#+caption: share/capture/org-mode/capture-meeting
...
#+caption: share/capture/org-mode/capture-cr
...
#+caption: share/capture/org-mode/capture-interview
...
#+begin_src emacs-lisp :tangle autoload/journal.el
;;;###autoload
...
#+caption: autoload/notes.el
#+begin_src emacs-lisp :tangle autoload/notes.el
;;;###autoload
...
#+caption: config.el
#+begin_src emacs-lisp
(defvar my-emacs-lisp-comment-fill-column 80
  "Define a different `fill-column' to use in lisp when writing comments.")
#+end_src

#+caption: autoload/lisp.el
#+begin_src emacs-lisp :tangle autoload/lisp.el
;;;###autoload
(defadvice! my-emacs-lisp-fill-comment-paragraph-a (orig-fun &rest args)
  "Set `fill-column' before calling `'fill-comment-paragraph'"
  :around 'fill-comment-paragraph
  (let ((fill-column my-emacs-lisp-comment-fill-column))
      (apply orig-fun args)))
#+end_src
...
The next change is to use our custom =variable-pitch-ui= font face. I have to replace the entire ~doom-themes-enable-treemacs-variable-pitch-labels~ function with a version that uses my custom font instead. To do that, I'll use Emacs advices:
...
#+begin_src emacs-lisp :tangle autoload/editor.el
;;;###autoload
(defun my-treemacs-setup-title-h ()
  (let ((bg (face-attribute 'default :background))
        (fg (face-attribute 'default :foreground)))
    (face-remap-add-relative 'header-line
                             :background bg :foreground fg
                             :box `(:line-width ,(/ +doom--line-pixel-height 2) :color ,bg)))
  (setq header-line-format
        '((:eval
           (concat
            " ☰ "
            (s-titleize (persp-current-name)))))))
#+end_src

#+begin_src emacs-lisp
  (add-hook 'treemacs-mode-hook #'my-treemacs-setup-title-h)
#+end_src
...
#+caption: autoload/work.el
#+begin_src emacs-lisp :tangle autoload/work.el
;;;###autoload
(defconst +doom-calendar-color-table (make-hash-table :test #'equal)
  "A hash table to hold an association between a calendar
name (in our org world, this is the event's category), and its
defined color.")
...
(defun my-work-calendar-color (name &optional prop)
  "Returns a string representing the color for PROP for the
...
(after! calfw
  (custom-theme-set-faces! 'doom-one
    ...
    `(cfw:face-select :foreground ,(doom-color 'orange-bg 256)
                      :background "#f0c774")
...
#+begin_src emacs-lisp :tangle autoload/work.el
;;;###autoload
(defun my-exclude-agenda-buffers-from-perspective-h nil
  "Don't associate temporary agenda buffers with current workspace."
  (if (and org-agenda-new-buffers
        ...
        (let (persp-autokill-buffer-on-remove)
          (mapc (lambda (buf)
                  (if (persp-buffer-in-other-p buf)
                      (bury-buffer buf)
                    (kill-buffer buf))) org-agenda-new-buffers)))))
#+end_src

#+caption: autoload/work.el
#+begin_src emacs-lisp :tangle autoload/work.el
;;;###autoload
(defun my-org-agenda-setup-buffer-h ()
...
(defadvice! my-org-agenda-reset-font-a (&rest _)
  "Reset text scale before running `org-agenda-redo' to prevent
messing with layout."
...
#+begin_src emacs-lisp :tangle autoload/work.el
;;;###autoload
(defadvice! my-org-agenda-always-split-horizontally-a (&rest _)
  :after 'org-prepare-agenda
  (my-window-toggle-split))
#+end_src

#+caption: autoload/work.el
#+begin_src emacs-lisp :tangle autoload/work.el
;;;###autoload
(defun my-org-agenda/switch-to-item ()
  "Go to the Org mode file which contains the item at point.
When optional argument DELETE-OTHER-WINDOWS is non-nil, the
displayed Org file fills the frame."
  (interactive)
  (let (buffer)
    (when-let ((target (or (org-get-at-bol 'org-target-file)
                           (org-agenda-error)))
               (file-name (car target))
               (pos (cdr target)))
      (if (setq buffer (get-file-buffer file-name))
...
(after! org-agenda
  ...
  (add-hook 'org-agenda-mode-hook #'my-org-agenda-setup-buffer-h)
  (add-hook 'org-agenda-finalize-hook #'my-org-agenda-setup-time-grid-h)
  (add-hook 'org-agenda-finalize-hook #'my-exclude-agenda-buffers-from-perspective-h)
...
(defadvice! my-org-agenda-make-agenda-icon-header-a (func name)
  ...
  (let ((icon (get-text-property 0 'icon name))
    ...
    (concat
     "\n"
     (when icon
       (format "%s " (my-get-icon icon icon-color)))
     header
     "\n"
     (if icon
        ...
        '(("c" "Super view"
           ((agenda "" ((org-agenda-span 'day)
                        ...
                        (org-super-agenda-groups
                         `((:name ,(my-org-agenda-group-title "Agenda" 'calendar (doom-color 'light-blue))
                            :time-grid t
                            :and (:date today
                                  :todo nil))))))
