...
;;;###autoload
(defun +snippets/delete-backward-char-or-field (&optional field)
...
(map! :after yasnippet
      ...
      (:map yas-keymap
       :g [backspace] #'+snippets/delete-backward-char-or-field
...
#+begin_src emacs-lisp :tangle autoload/editor.el :mkdirp yes
;;;###autoload
(defun +completion/yasnippet-or-completion ()
  (interactive)
...
#+begin_src emacs-lisp :tangle autoload/editor.el :mkdirp yes
;;;###autoload
(defun +completion/complete-selection-or-return ()
  (interactive)
...
(after! company
  ...
  (map! :map company-active-map
        :g [tab] #'+completion/yasnippet-or-completion
        :g [backtab] #'company-select-previous
        :g [down] #'company-select-next
        :g [up] #'company-select-previous
        :g [return] #'+completion/complete-selection-or-return
...
(add-hook! 'doom-load-theme-hook
  (defun +doom-themes-add-extra-colors-h ()
    (pushnew! doom-themes--colors
              '(green-bg "#435300" "#435300" "black")
              '(violet-bg "#4C2E60" "#4C2E60" "black")
              '(yellow-bg "#644E00" "#644E00" "black")
              '(blue-bg "#1B2E47" "#1B2E47" "black")
              '(orange-bg "#492812" "#492812" "black")
              '(red-bg "#360F0F" "#360F0F" "black")
              '(magenta-bg "#645867" "#645867" "black")
              '(light-blue "#a0b9ce" "#a0b9ce" "brightblue")
              '(light-blue-bg "#3B454D" "#3B454D" "black")
              '(dark-blue-bg "#14232C" "#14232C" "black")
              '(cyan-bg "#354B48" "#354B48" "black")
              '(dark-cyan-bg "#1C2725" "#1C2725" "black")
              '(bright-violet "#d6c6db" "#d6c6db" "brightmagenta")
              '(bright-blue "#c0d0de" "#c0d0de" "bright")
              '(bright-magenta "#e3d9e7" "#e3d9e7" "brightmagenta")
              '(bright-light-blue "#e5ecf2" "#e5ecf2" "brightblue"))))
#+end_src
...
#+caption: autoload/appearances.el
#+begin_src emacs-lisp :tangle autoload/appearances.el :mkdirp yes
;;; autoload/appearances.el -*- lexical-binding: t; -*-
...
(use-package! mixed-pitch
  :defer t
  :init
  (add-hook! (Info-mode lexic-mode)
    (defun +doom-mixed-pitch-setup-h ()
      (mixed-pitch-mode 1)
        ...
        (dolist (face mixed-pitch-fixed-pitch-faces)
          (add-to-list 'mixed-pitch-fixed-cookie
                       (face-remap-add-relative face
                                                ...
                                                :height fix-height))))))
  :config
  (setq mixed-pitch-fixed-pitch-faces (append mixed-pitch-fixed-pitch-faces '(font-lock-comment-face))))
#+end_src
...
(use-package! info-plus
  :commands Info-toggle-fontify-all
  :init
  (add-hook! 'Info-selection-hook
    (defun +doom-info--fontify-info-h ()
...
(after! info
  (custom-theme-set-faces! 'doom-tomorrow-night
    `(info-title-1 :foreground ,(doom-color 'blue 256)
                   :height 1.8
                   :weight normal
                   :slant normal
                   :inherit variable-pitch)
    `(info-title-2 :foreground ,(doom-color 'magenta 256)
                   :height 1.7
                   :weight normal
                   :slant italic
                   :inherit variable-pitch)
    `(info-title-3 :foreground ,(doom-color 'violet 256)
                   :height 1.6
                   :weight normal
                   :slant italic
                   :inherit variable-pitch)
    `(info-title-4 :foreground ,(doom-color 'light-blue 256)
                   :height 1.5
                   :weight bold
                   :slant normal
                   :inherit variable-pitch)
    `(info-menu-header :foreground ,(doom-color 'magenta 256)
                       :height 1.3
                       :weight bold
                       :slant normal
                       :inherit variable-pitch)
    ...
    `(info-glossary-word :foreground ,(doom-color 'yellow 256)
                         :background ,(doom-color 'yellow-bg 256)
                         :box (:line-width -1 :color ,(doom-color 'yellow 256) :style released-button)
                         :inherit fixed-pitch)
...
(defconst +doom-supported-icons
  '(assign
    ...
    visibility
    wait)
  "List of all iconse supported on captur templates.")
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/appearances.el :mkdirp yes
;;;###autoload
(defun +doom-get-raw-icon (icon)
  "This function returns the requested ICON as if you're calling
the `all-the-icons-'* functions directly.

Currrently, supported icons are:
...
"
  (pcase icon
    ('assign (all-the-icons-material "assignment_ind" :height 0.95 :v-adjust -0.17))
    ...
    ('calendar (all-the-icons-octicon "calendar" :height 1.05 :v-adjust 0.1))
...
#+begin_src emacs-lisp :tangle autoload/appearances.el :mkdirp yes
;;;###autoload
(defun +doom-get-icon (icon &optional color)
  "This function returns the requested ICON as a 2-characters
string, propertized to dieplsy the associated icon frorm the
`all-the-icons' package. It uses the COLOR arg to 'paint' the
icon.

Check documentation of the function `+doom-get-raw-icon' for a
list of supported icons."
  (cond ((eq color 'raw) (+doom-get-raw-icon icon))
        (color (propertize "--"
                   'face `(:foreground ,color)
                   'display (+doom-get-raw-icon icon)))
        (t (propertize "--"
...
(set-popup-rules!
  '(("^\\*" :slot 2 :vslot -1 :side bottom :size 0.4 :select t)
    ("^ \\*" :slot 2 :vslot -1 :side bottom :size +popup-shrink-to-fit)
    ("^\\*Completions" :ignore t)
    ("^\\*Local variables\\*$"
     :slot 2     :vslot -1   :side bottom :size +popup-shrink-to-fit
     :select nil :modeline t :autosave t  :ttl 0 :quit t)
    ("^\\*\\(?:[Cc]ompil\\(?:ation\\|e-Log\\)\\|Messages\\)"
     :slot 2     :vslot -1   :side bottom :width 110 :height 0.4
     :select nil :modeline t :autosave t  :ttl nil   :quit t)
    ("^\\*\\(?:doom \\|Pp E\\)"
     :slot 2     :vslot -1   :side bottom :size +popup-shrink-to-fit
     :select nil :modeline t :autosave t  :ttl 0 :quit t)
    ("^\\*doom:"
     :slot 2   :vslot -1   :side bottom :width 110 :height 0.4
     :select t :modeline t :autosave t  :ttl t     :quit nil)
    ("^\\*doom:\\(?:v?term\\|e?shell\\)-popup"
     :slot 2   :vslot -1     :side bottom  :width 110 :height 0.4
     :select t :modeline nil :autosave nil :ttl nil   :quit nil)
    ("^\\*\\(?:Wo\\)?Man "
     :slot 2   :vslot -1     :side right  :width 110 :height 0.4
     :select t :modeline nil :autosave nil :ttl 0     :quit t)
    ("^\\*Calc"
     :slot 2   :vslot -1     :side bottom  :width 110 :height 0.4
     :select t :modeline nil :autosave nil :ttl 0     :quit nil)
    ("^\\*eww\\*"
     :slot 2   :vslot -1     :side right   :width 0.4 :height 0.4
     :select t :modeline nil :autosave nil :ttl 0     :quit t)
    ("^\\*Customize"
     :slot 2   :vslot -1     :side right   :width 0.4 :height 0.4
     :select t :modeline nil :autosave nil :ttl 0     :quit nil)
    ("^\\*\\(?:doom:\\)?scratch\\*"
     :slot 2   :vslot -1     :side right   :width 0.4 :height 0.4
     :select t :modeline nil :autosave nil :ttl nil   :quit t)
    ("^ \\*undo-tree\\*"
     :slot 2   :vslot -1     :side left    :width 20 :height 0.5
     :select t :modeline nil :autosave nil :ttl 0    :quit t)
    ("^\\*[Hh]elp"
    ...
    ("^\\*Warnings"
     :slot 2     :vslot -1     :side bottom  :width 110 :height 0.4
     :select nil :modeline nil :autosave nil :ttl nil   :quit t)
    ("^\\*Backtrace"
     :slot 2     :vslot -1     :side bottom  :width 110 :height 0.4
     :select nil :modeline nil :autosave nil :ttl nil   :quit nil)
    ("^\\*CPU-Profiler-Report "
     :slot 2     :vslot -1     :side bottom  :width 110 :height 0.4
     :select nil :modeline nil :autosave nil :ttl 0     :quit nil)
...
(add-hook! (help-mode
            ...
            Info-mode
            lexic-mode)
  (defun +popup-format-info-side-pannel-h ()
...
(use-package! orderless
  ...
  :config
  (custom-theme-set-faces! 'doom-tomorrow-night
    `('orderless-match-face-0 :background ,(doom-color 'green-bg 256) :foreground ,(doom-color 'green 256))
    `('orderless-match-face-1 :background ,(doom-color 'violet-bg 256) :foreground ,(doom-color 'violet 256))
    `('orderless-match-face-2 :background ,(doom-color 'yellow-bg 256) :foreground ,(doom-color 'yellow 256))
    `('orderless-match-face-3 :background ,(doom-color 'blue-bg 256) :foreground ,(doom-color 'blue 256))))
#+end_src
...
#+caption: config.el
#+begin_src emacs-lisp :padline no
  (custom-theme-set-faces! 'doom-tomorrow-night `(selectrum-current-candidate :background ,(doom-color 'base4)))
#+end_src
...
The command supports recent files, bookmarks, views and project
files as virtual buffers. Buffers are previewed. Furthermore
narrowing to buffers (b), files (f), bookmarks (m), views (v) and
project files (p) is supported via the corresponding keys. In
order to determine the project-specific files and buffers, the
`consult-project-root-function' is used. The list of available
...
Depending on the selected item OPEN-BUFFER, OPEN-FILE or
OPEN-BOOKMARK will be used to display the item."
...
(defun +doom-adjust-outline-fonts-h ()
  (after! outline
    (custom-theme-set-faces! 'doom-tomorrow-night
     `(outline-1 :foreground ,(doom-color 'blue 256)
                 :height 2.0
                 :weight normal
                 :slant normal
                 :inherit variable-pitch)
     `(outline-2 :foreground ,(doom-color 'magenta 256)
                 :height 1.8
                 :weight normal
                 :slant italic
                 :inherit variable-pitch)
     `(outline-3 :foreground ,(doom-color 'violet 256)
                 :height 1.6
                 :weight normal
                 :slant italic
                 :inherit variable-pitch)
     `(outline-4 :foreground ,(doom-color 'light-blue 256)
                 :height 1.4
                 :weight bold
                 :slant normal
                 :inherit variable-pitch)
     `(outline-5 :foreground ,(doom-color 'bright-violet 256)
                 :height 1.4
                 :weight bold
                 :slant normal
                 :inherit variable-pitch)
     `(outline-6 :foreground ,(doom-color 'bright-blue 256)
                 :height 1.2
                 :weight bold
                 :slant normal
                 :inherit variable-pitch)
     `(outline-7 :foreground ,(doom-color 'bright-magentann/ 256)
                 :height 1.2
                 :weight bold
                 :slant normal
                 :inherit variable-pitch)
     `(outline-8 :foreground ,(doom-color 'bright-light-blue 256)
                 :height 1.1
                 :weight bold
                 :slant normal
                 :inherit variable-pitch))
...
(defun +doom-adjust-org-fonts-h ()
  (after! org
    (custom-set-faces!
      ...
      `(org-drawer :foreground ,(doom-color 'doc-comments) :inherit fixed-pitch)
...
#+begin_src emacs-lisp :tangle autoload/org.el :mkdirp yes
;;;###autoload
(defun +org/link-snippet-in-place ()
  (interactive)
...
(map! :after org
      (:map org-mode-map
       :v "s-k" #'+org/link-snippet-in-place))
#+end_src
...
#+begin_src emacs-lisp
(defconst +file-templates-yasnippet-dir (expand-file-name "share/templates" doom-private-dir))
(pushnew! yas-snippet-dirs '+file-templates-yasnippet-dir)
#+end_src


#+caption: share/templates/org-mode/__
#+begin_src snippet :tangle share/templates/org-mode/__ :mkdirp yes
# -*- mode: snippet -*-
# name: Blank Org File
# key: __
# --
...
[[${1:$$(unless (or yas-moving-away-p yas-modified-p)
  (let ((links (+org-link--links-in-kill-ring)))
    (if links
        (yas-auto-next
...
#+begin_src emacs-lisp
(defconst +org-journal-dir (concat org-directory "/journal/")
  "Path to directory holding all journal files.")
#+end_src
...
(use-package org-journal
  ...
  (setq org-journal-encrypt-journal nil
        ...
        org-journal-file-format +org-journal-file-format
  ...
  (font-lock-add-keywords 'org-journal-mode +doom-org-font-lock-beautify-keywords)
  (set-window-buffer nil (current-buffer)))
#+end_src

#+caption: autoload/journal.el
#+begin_src emacs-lisp :tangle autoload/journal.el :mkdirp yes
;;; $DOOMDIR/autoload/org-journal.el -*- lexical-binding: t; -*-

;;;###autoload
(defun +org-journal/new-entry (&optional prefix add-header-p variable make-local)
  "Open a journal file and start a new entry. Without any
arguments, this function is equivalent to
calling `(org-journal-new-entry t)'

With PREFIX argumant, request the journal timestamp to the user.

If ADD-HEADER-P is non-nil, fully create the new entry (this is
equivalent to `(not PREFIX)' on the `org-journal-new-entry').

VARIABLE is a string that, when set, will store the timestamp
value selected by the user. This function ignores this argument
if PREFIX is nil.

When MAKE-LOCAL is a non-nil value, this function uses VARIABLE
to create a local variable. If PREFIX or VARIABLE is nil, this
...
#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;; autoload/org.el *- lexical-binding: t; -*-

(defconst +org-capture--note-frame-height 15
  "Number of lines a note frame should have when capturing a long
note.")

(defconst +org-capture--long-note-frame-height 35
  "Number of lines a note frame should have when capturing a long
note.")

(defconst +org-capture--menu-width 50
  "Number of lines a note frame should have when capturing a long
note.")

(defvar +org-capture--yas-prefix "--DOOM-ORG-CAPTURE-YAS-PREFIX--:"
  "String used to identify a yasnippet template on a capture
buffer")

(defvar +org-capture--source-buffer nil
  "Stores the buffer object where the capture started")

(defvar +org-capture--source-selection nil
  "Stores the buffer object where the capture started")

(defvar +org-capture--timestamp nil
  "Stores the time when the capture started")
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture--yas-expand-h ()
  "Expand `+org-capture--yas-prefix' on the template."
  (setq +org-capture--auto-align-tags-store org-auto-align-tags)
  (let ((saved-point (point)))
    (goto-char (point-min))
    (if (search-forward-regexp (concat +org-capture--yas-prefix "\\([^:]*\\):") nil t)
        (progn
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture--finish-h ()
  "Clean up any outstanding variable and values set when capture
starts."
  (+org-capture-clear-journal-timestamp)
  (setq org-auto-align-tags +org-capture--auto-align-tags-store)
  (setq +org-capture--source-buffer nil
        +org-capture--source-selection nil
        +org-capture--auto-align-tags-store nil))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture-clear-journal-timestamp ()
  "Unbound the capture variable returned by
`+org-capture-last-journal-timestamp'. If you used the
`+org-capture-journal-file' function, you should call this
function when the capture is done to guarantee that on the next
capture you're not inputting old values from your template."
  (when (boundp '+org-capture-timestamp)
    (makunbound '+org-capture-timestamp)))
#+end_src
...
(use-package! org-capture
  ...
  :init
  (add-hook 'org-capture-mode-hook '+org-capture--yas-expand-h)
  (add-hook 'org-capture-after-finalize-hook '+org-capture--finish-h)
  :config
  (setq +org-capture-changelog-file "CHANGELOG.org")
  (setq org-capture-templates-contexts '(("p" (+org-capture-projects-p))))
  ...
  (map! :map org-capture-mode-map
        :gin "C-RET" #'org-capture-finalize
        :gin [s-escape] #'org-capture-kill)
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-projects-p ()
  (with-current-buffer (or +org-capture--source-buffer (current-buffer))
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-description (&rest args)
  "Returns a string to be used as the description of a capture
item.
...
Example:
  ...
  (+org-capture-description :text \"Add journal entry\"
                            :icon '(journal . \"#ff00ff\"))"
    ...
    (if icon
        (progn
          (unless (and (consp icon)
                       (memq (car-safe icon) +doom-supported-icons)
                       (stringp (cdr-safe icon)))
            (error (concat "A capture description icon must be a cons cell with"
                           " one of +doom-supported-icons symbol and a string color.")))
          (format "%s %s" (+doom-get-icon (car icon) (cdr icon)) text))
      text)))
#+end_src
...
#+begin_src emacs-lisp
  (pushnew! org-capture-templates
            `("n"  ,(+org-capture-description :text "Fleet note" :icon `(notes . ,(doom-color 'green)))
              plain (function +org-capture-journal-file)
              ,(concat "** %(+org-capture-last-journal-timestamp)%?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t))
#+end_src
...
(defun +org-capture-journal-file ()
  "Thin wrapper around `+org-journal-new-entry' that adjust the
capture frame width to `fill-column' before start editing it."
  (+org-capture--adjust-frame)
  (+org-journal/new-entry))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--adjust-frame (&optional extended-p frame)
  "Dinamically adjust the capture FRAME width to WIDTH and
reposition it on the center of the screen."
  (when (+org-capture-frame-p)
    (set-frame-size (or frame (selected-frame))
                    (+ fill-column 20)
                    (if extended-p
                        +org-capture--long-note-frame-height
                      +org-capture--note-frame-height))
    (+org-capture--reposition-frame frame)))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--reposition-frame (&optional frame)
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-last-journal-timestamp ()
  "Returns the last timestamp captured for the org journal,
already formatted with the format specified on
  `org-journal-time-format'. If user didn't capture any journal
  date, returns the current time."
  (format-time-string org-journal-time-format +org-capture--timestamp))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-template-selection ()
  "Returns the text selected on the buffer that originated the
capture. If the selection is the entire buffer, return an org
link to buffer."
  (with-current-buffer +org-capture--source-buffer
    (when (region-active-p)
      (or (+org-capture--mu4e-headers-selected)
          (+org-capture--mu4e-view-selected)
          (+org-capture--buffer-selected)
          ""))))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--mu4e-headers-selected (&optional buf-name)
  "When on a mu4e-headers buffer, store an org link to each one
of the selected messages and return them all as a string."
...
#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--mu4e-view-selected (&optional buf-name)
  "When on a mu4e-view buffer, store an org link to the message
and return it as a string with the selected text as a
block-quote. If the selection is the entire buffer, just return
the org link."
...
#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--buffer-selected ()
  "When there is an active selection in the current buffer,
returns an org link to the current buffer with the selection as a
blockquote. If the selection is the entire buffer, just return
the org link."
...
#+begin_src emacs-lisp
  (pushnew! org-capture-templates
            `("j"  ,(+org-capture-description :text "Journal" :icon `(journal . ,(doom-color 'blue 256))))
            `("js" ,(+org-capture-description :text "Support" :icon `(help . ,(doom-color 'orange 256)))
              plain (function +org-capture-journal-file-for-meeting)
              ,(concat "** %(+org-capture-last-journal-timestamp)%(+org-capture-yasnippet 'capture-support)%?\n"
                       "%(+org-capture-template-selection)")
              :clock-in t
              :kill-buffer t)
            `("jm" ,(+org-capture-description :text "Meeting" :icon `(meeting . ,(doom-color 'cyan 256)))
              plain (function +org-capture-journal-file-for-meeting)
              ,(concat "** %(+org-capture-last-journal-timestamp)%(+org-capture-yasnippet 'capture-meeting)%?\n"
                       "%(+org-capture-template-selection)")
              :clock-in t
              :kill-buffer t)
            `("jc" ,(+org-capture-description :text "Code review" :icon `(review . ,(doom-color 'green 256)))
              plain (function +org-capture-journal-file-for-meeting)
              ,(concat "** %(+org-capture-last-journal-timestamp)%(+org-capture-yasnippet 'capture-cr)%?\n"
                       "%(+org-capture-template-selection)")
              :clock-in t
              :kill-buffer t)
            `("ji" ,(+org-capture-description :text "Idea" :icon `(idea . ,(doom-color 'yellow 256)))
              plain (function +org-capture-journal-file)
              ,(concat "** %(+org-capture-last-journal-timestamp)%(+org-capture-yasnippet 'capture-idea)%?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("jl" ,(+org-capture-description :text "Interesting link" :icon `(globe . ,(doom-color 'blue 256)))
              plain (function +org-capture-journal-file)
              ,(concat "** %(+org-capture-last-journal-timestamp)%(+org-capture-yasnippet 'capture-link)%?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("jp" ,(+org-capture-description :text "Past entry" :icon `(history . ,(doom-color 'violet 256)))
              plain (function +org-capture-journal-file-for-timestamp)
              ,(concat "** %(+org-capture-last-journal-timestamp)%?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-journal-file-for-meeting ()
  "Thin wrapper around `+org-journal-new-entry' that adjust the
capture frame width to `fill-column', and give a bigger height to
add meeting notes."
  (+org-capture--adjust-frame t)
  (+org-journal/new-entry))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-yasnippet (snippet)
  "Returns a snippet mark with the given SNIPPET used to expand
it with yasnippet when the capture finishes.
...
#+caption: share/capture/org-journal-mode/capture-meeting
#+begin_src snippet :tangle share/capture/org-journal-mode/capture-meeting :mkdirp yes
# -*- mode: snippet -*-
# name: New meeting notes
# key: capture-meeting
# --
${1:Meeting topic}  ${2:`":"`${3:meeting}`":"`}
...
#+caption: config.el
#+begin_src emacs-lisp
(defconst +org-capture-yasnippet-dir (expand-file-name "share/capture" doom-private-dir)
  "Directory containning all capture snippets.")
(pushnew! yas-snippet-dirs '+org-capture-yasnippet-dir)
#+end_src

#+caption: share/capture/org-journal-mode/capture-support
#+begin_src snippet :tangle share/capture/org-journal-mode/capture-support :mkdirp yes
# -*- mode: snippet -*-
# name: New support request notes
# key: capture-support
# --
Support about ${1:Support topic}  ${2:`":"`${3:support}`":"`}
- Requested by ${4:Coworker Name}
- $0
...
#+caption: share/capture/org-journal-mode/capture-cr
#+begin_src snippet :tangle share/capture/org-journal-mode/capture-cr :mkdirp yes
# -*- mode: snippet -*-
...
#+caption: share/capture/org-journal-mode/capture-idea
#+begin_src snippet :tangle share/capture/org-journal-mode/capture-idea :mkdirp yes
# -*- mode: snippet -*-
# name: New Idea
# key: capture-idea
# --
...
#+caption: share/capture/org-journal-mode/capture-link
#+begin_src snippet :tangle share/capture/org-journal-mode/capture-link :mkdirp yes
# -*- mode: snippet -*-
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-journal-file-for-timestamp ()
  "Asks user for a specific timestamp and returns the journal
  file for it. It also position the cursor on the correct place
  for a new entry, and adjust the capture frame width to
  `fill-column' before start editing."
  (+org-capture--adjust-frame)
  (+org-journal/new-entry t nil '+org-capture-timestamp))
#+end_src
...
#+begin_src emacs-lisp
  (pushnew! org-capture-templates
            `("t"  ,(+org-capture-description :text "Tasks" :icon `(inbox . ,(doom-color 'orange 256))))
            `("tt" ,(+org-capture-description :text "General task" :icon `(inbox . ,(doom-color 'green 256)))
              plain (function +org-capture-journal-file)
              ,(concat "** TODO %?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("td" ,(+org-capture-description :text "Task with deadline" :icon `(timer . ,(doom-color 'yellow 256)))
              plain (function +org-capture-journal-file-for-task-with-deadline)
              ,(concat "** TODO %?\n"
                       "DEADLINE: %(+org-capture-last-time-read)\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("ts" ,(+org-capture-description :text "Scheduled task" :icon `(calendar . ,(doom-color 'orange 256)))
              plain (function +org-capture-journal-file-for-task-with-schedule)
              ,(concat "** TODO %?\n"
                       "SCHEDULED: %(+org-capture-last-time-read)\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t))
#+end_src
...
#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture-journal-file-for-task-with-date (title)
  "Asks user for a timestamp and store it for template use. It
also position the cursor on the correct place for a new entry,
and adjust the capture frame width to `fill-column' before start
editing."
  (+org-capture--adjust-frame)
  (setq +org-capture-timestamp (org-read-date t t nil title))
  (+org-journal/new-entry))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-journal-file-for-task-with-deadline ()
  "Asks user for a timestamp for the task's deadline."
  (+org-capture-journal-file-for-task-with-date "Deadline"))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-journal-file-for-task-with-schedule ()
  "Asks user for a timestamp for the task's start time"
  (+org-capture-journal-file-for-task-with-date "Start time"))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-last-time-read ()
  "Returns the last timestamp captured,formatted with the format
specified on `org-time-stamp-formats'. If user didn't capture any
journal date, returns the current time."
  (format-time-string (cdr-safe org-time-stamp-formats)
                      (when (boundp '+org-capture-timestamp) +org-capture-timestamp)))
#+end_src
...
#+begin_src emacs-lisp
  (pushnew! org-capture-templates
            `("c" ,(+org-capture-description :text "Contact" :icon `(contact-add . ,(doom-color 'yellow 256)))
              plain (file (+org-capture-contacts-file))
              ,(concat "* %(+org-capture-yasnippet 'capture-contact)\n"
                       "%?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-contacts-file ()
...
#+begin_src emacs-lisp
  (pushnew! org-capture-templates
            `("p"  ,(+org-capture-description :text "Projct" :icon `(project . ,(doom-color 'violet 256))))
            `("pt" ,(+org-capture-description :text "New task" :icon `(checklist . ,(doom-color 'green 256)))
              entry (file+headline +org-capture-project-notes-file-wrapper "Tasks")
              ,(concat "* TODO %?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("pn" ,(+org-capture-description :text "Quick note" :icon `(postit . ,(doom-color 'yellow 256)))
              entry (file+headline +org-capture-project-notes-file-wrapper "Notes")
              ,(concat "* %U %?\n"
                       "%(+org-capture-template-selection)")
              :kill-buffer t)
            `("pc" ,(+org-capture-description :text "New changelog" :icon `(list . ,(doom-color 'violet 256)))
              item (function +org-capture-project-changelog-file-with-headings)
              "- %?"
              :prepend t
              :kill-buffer t))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-project-notes-file-wrapper ()
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture-project-changelog-file-with-headings ()
  (let ((entry-path (+org-capture-project-changelog-file))
    ...
    (goto-char (point-min))
    (+org-capture--ensure-heading-prepend (+org-capture-project-target-heading))))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture-project-target-heading ()
  `("Unreleased" ,(format-time-string "%A, %B %-e, %Y")))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
(defun +org-capture--ensure-heading-prepend (headings &optional initial-level)
  (if (not headings)
      (widen)
    (let ((initial-level (or initial-level 1)))
      ...
      (+org-capture--ensure-heading-prepend (cdr headings) (1+ initial-level)))))
#+end_src
...
#+begin_src emacs-lisp
  (map! :leader
        :desc "Org Capture"           "X" #'+org-capture/same-frame
        (:prefix-map ("n" . "notes")
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defadvice! +org-capture-open-frame-get-info-a (&rest _)
  "Store the buffer object where the user initiated the capture."
  :before '+org-capture/open-frame
  (setq +org-capture--source-buffer (current-buffer)
        +org-capture--source-selection (evil-visual-range)))
#+end_src

#+caption: autoload/capture.el
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defun +org-capture/same-frame()
  (interactive)
  (+org-capture-open-frame-get-info-a)
  (org-capture))
#+end_src
...
#+begin_src emacs-lisp :tangle autoload/capture.el :mkdirp yes
;;;###autoload
(defadvice! +org-capture-select-template-prettier-a (&optional keys)
  "Select a capture template, in a prettier way than default Lisp
programs can force the template by setting KEYS to a string."
  ...
  (let ((org-capture-templates
    ...
    (if keys
      ...
      (org-mks org-capture-templates
               ...
               " "
               `(("q" ,(+org-capture-description :text "Abort" :icon `(stop . ,(doom-color 'red 256)))))))))


;;;###autoload
(defadvice! +org-capture-mks-prettier-a (table title &optional prompt specials)
  "Select a member of an alist with multiple keys. Prettified.

TABLE is the alist which should contain entries where the car is
a string. There should be two types of entries.

1. prefix descriptions like (\"a\" \"Description\") This
   indicates that `a' is a prefix key for multi-letter selection,
   and that there are entries following with keys like \"ab\",
   \"ax\"…

2. Select-able members must have more than two elements, with the
   first being the string of keys that lead to selecting it, and
   the second a short description string of the item.

The command will then make a temporary buffer listing all entries
that can be selected with a single key, and all the single key
prefixes. When you press the key for a single-letter entry, it is
selected. When you press a prefix key, the commands (and maybe
further prefixes) under this key will be shown and offered for
selection.
              ...
              (let ((original-evil-normal-state-cursor evil-normal-state-cursor)
                      ...
                      (`(,(and key (pred (string-match re))) ,desc)
                       (let ((k (match-string 1 key)))
                         ...
                         (if (member k tab-alternatives)
                             (push "\t" allowed-keys)
                           (push k allowed-keys))
                         (insert (+org-capture--item-entry k desc current t) "\n")))
                      ;; Usable entry.
                      (`(,(and key (pred (string-match re))) ,desc . ,_)
                       (let ((k (match-string 1 key)))
                         (insert (+org-capture--item-entry k desc current) "\n")
                         (push k allowed-keys)))
                      (_ nil))))
                ...
                (when specials
                  ...
                  (pcase-dolist (`(,key ,description) specials)
                    (insert (+org-capture--item-entry key
                                                      description
                                                      (when current
                ...
                ;; a sub-level prefix.
                (goto-char (point-min))
                (+org-capture--refresh-capture-frame)
...
(defun +org-capture--item-entry (key description &optional prefix parent-option-p key-face-extras)
  "Returns a string representing an Org Capture item with the
given KEY and DESCRIPTION.

PREFIX is the character sequences already pressed during the
capture process.

If PARENT-OPTION-P is non-nil, it means the this entry has
sub-entries and some sort of indication should be displayed

The KEY-FACE-EXTRAS is one or more simbols to be added to the KEY
face on the entry."
...
(defun +org-capture--refresh-capture-frame ()
  "This function is called right after all the capture options
were written in the capture buffer."
  (when (+org-capture-frame-p)
    ...
    (set-frame-size frame
                    +org-capture--menu-width
                    (+ (line-number-at-pos (point-max)) 3))
    (+org-capture--reposition-frame)
    (message ""))))
#+end_src
...
#+begin_src snippet :tangle share/capture/org-mode/capture-contact :mkdirp yes
# -*- mode: snippet -*-
# name: New Contact
# key: capture-contact
# --
...
(after! org
  ...
  (set-popup-rule! "^\\*Org Agenda"
    :slot 2   :vslot -1     :side 'right   :width 100 :height 0.4
    :select t :modeline nil :autosave t   :ttl nil   :quit 'current)
  (setq org-agenda-files (list (concat org-directory "/agenda/tasks/todo.org" )
                               (concat +org-journal-dir (format-time-string +org-journal-file-format)))
        org-archive-location (concat org-directory "/.archives/%s::datetree/* Archived From %s")
...
#+begin_src emacs-lisp :tangle autoload/org.el :mkdirp yes
;;;###autoload
(defun +org-exclude-agenda-buffers-from-perspective-h nil
  "Don't associate temporary agenda buffers with current workspace."
  (if
      (and org-agenda-new-buffers
           (and (boundp 'persp-mode) persp-mode)
           (not org-agenda-sticky))
      (progn
        (let (persp-autokill-buffer-on-remove)
          (mapc 'persp-remove-buffer org-agenda-new-buffers)))))
#+end_src
...
(after! org-agenda
  ...
  (add-hook 'org-agenda-finalize-hook '+org-exclude-agenda-buffers-from-perspective-h)
  )
#+end_src

*** TODO Modify org agenda key map to use 'j' and 'k' to move between items
...
#+begin_src emacs-lisp :tangle autoload/org.el :mkdirp yes
;;;###autoload
(defadvice! +org-agenda-make-agenda-icon-header-a (func name)
  ...
  (let ((icon (get-text-property 0 'icon name))
    ...
    (concat
     "\n"
     (when icon
       (format "%s " (+doom-get-icon icon icon-color)))
     header
     "\n"
     (if icon
...
#+begin_src emacs-lisp :tangle autoload/org.el :mkdirp yes
;;;###autoload
(defun +org-agenda-group-title (text &optional icon icon-color)
...
(use-package! org-super-agenda
  ...
  (add-hook! org-agenda-mode
    (setq-local wrap-prefix (s-repeat 22 " "))
    (variable-pitch-mode 1))
           ...
           ((agenda "" ((org-agenda-span 'day)
            ...
            (alltodo "" ((org-agenda-overriding-header "")
                         (org-super-agenda-groups
                          `(
                            (:name ,(+org-agenda-group-title "Inbox" 'inbox (doom-color 'orange))
                             :category "journal"
                             :order 15)
                            (:name ,(+org-agenda-group-title "Delegated" 'assign (doom-color 'orange))
                             :todo "DELG"
                             :order 45)
                            (:name ,(+org-agenda-group-title "Waiting" 'wait  (doom-color 'yellow))
                             :todo "WAIT"
                             :order 35)
                            (:name ,(+org-agenda-group-title "On Hold" 'pause (doom-color 'yellow))
                             :todo "HOLD"
                             :order 40)
                            (:name ,(+org-agenda-group-title "Issues" 'bug (doom-color 'red))
                             :tag "issue"
                             :order 30)
                            (:name ,(+org-agenda-group-title "Habits" 'loop (doom-color 'blue))
                             :habit t
                             :order 1)
                            (:name ,(+org-agenda-group-title "Overdue" 'busy (doom-color 'red))
                             :deadline past
                             :face 'all-the-icons-dpink
                             :order 4)
                            (:name ,(+org-agenda-group-title "Due Today" 'new (doom-color 'orange))
                             :deadline today
                             :face 'all-the-icons-orange
                             :order 5)
                            (:name ,(+org-agenda-group-title "Actionable" 'checked (doom-color 'green))
                             :and (:todo ("TODO" "STRT")
                                   :scheduled (past today)
                                   :deadline nil)
                             :order 21)
                            (:name ,(+org-agenda-group-title "Due Soon" 'snooze (doom-color 'yellow))
                             :deadline future
                             :face 'all-the-icons-yellow
                             :order 10)
                            (:name ,(+org-agenda-group-title "Important" 'error (doom-color 'red))
                             :priority "A"
                             :order 15)
                            (:name ,(+org-agenda-group-title "Next to do" 'visibility (doom-color 'yellow))
                             :todo "NEXT"
                             :order 20)
                            (:name ,(+org-agenda-group-title "To do" 'checked (doom-color 'green))
                             :anything t
                             :order 23)
                            ))))))))
  :config
