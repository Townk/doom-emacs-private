...
A long time ago I discovered [[http://literateprogramming.com][Literate Programming]] when I first read an Emacs configuration using Org Mode. It was the [[https://pages.sachachua.com/.emacs.d/Sacha.html][Emacs configuration]] of [[https://sachachua.com/blog/about/][Sacha Chua]], and it was a fantastic reading. I tried couple times to do the same with past configurations without much success.
...
There are four potential loading /spots/ for each configuration you want to tweak, and you must always decide which one is the best option to apply your particular case:
...
- Inside the =:init= section of ~use-package~ :: Depending on how a package gets configured, this is equivalent to configuring something directly on the =config.el=, but not always. If a package configuration uses the =:after= section from ~use-package~, Emacs evaluates the code inside =:init= only *after* the resource stated on the option loads;
- Inside the =:config= section of ~use-package~ :: This always execute after the given package gets loaded. You need to consider other instances of the call to ~use-package~ on the same package. The order in which they evaluate dictates who will overwrite a variable last;
...
If you have any question about where you should add your configuration, I suggest following the following rule of thumb:
...
*** Modal editing
I need to come clean to the reader about my experiences with text editors. I'm a /die-hard/ Vim user, even though I'm not using Vim for some time now. Nevertheless, I personally believe that modal editing is far superior way to use a text editor than any other alternative, and that is why I consider /modal editing/, a feature that any text editor should have.
...
I move my editor all around my desktop, and I resize it just as much, do I'm not particular to any specific size or position it should be, but if I turn it off, the next time I decide to open it, I expect it to be in the same place, and with the same size I defined on the last time I've opened it.
...
*** Fonts
Fonts for writing must use variable pitch and follow the typography rules defined on section [[Typography]]. Any reference to code or code elements must get the normal /coding/ style with fixed pitch. Also, other then headlines, element should have variable size depending on their importance.
