...
On Emacs (and most of modern editors out there), the user has the ability to interact with the editor interactively. This is what the /Command Palette/ is on Sublime Edit, the /Command Line/ on Vim, and the /minibuffer/ on Emacs.

My editor, has to expose APIs and integrate the completion framework with all plugins/packages installed, and its behavior must be consistent throughout the application.

I also like to act on individual results, and have a simple syntax to allow me to narrow my completion options more intelligently.
